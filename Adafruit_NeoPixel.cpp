...
Adafruit_NeoPixel::Adafruit_NeoPixel(uint16_t n, uint8_t t) {
  numBytes = n * 3;
  if((pixels = (uint8_t *)malloc(numBytes))) {
    ...
    type    = t;
    pin     = STRING_LIGHTS;
    port    = portOutputRegister(digitalPinToPort(STRING_LIGHTS));
    pinMask = digitalPinToBitMask(STRING_LIGHTS);
    endTime = 0L;
  } else {
    numLEDs = 0;
  }
